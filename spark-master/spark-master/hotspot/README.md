...
To reduce the computation power needed we made the following changes:
...
1. Replace the logic of User Defined Functions ST\_Contains and ST\_Within in SpatialQuery.scala.
1. Append ```.master("local[*]")``` after ```.config("spark.some.config.option", "some-value")``` to tell IDE the master IP is localhost.
1. In some cases, you may need to go to "build.sbt" file and change ```% "provided"``` to ```% "compile"``` in order to debug your code in IDE
1. Run your code in IDE
1. **You must revert Step 3 and 4 above and recompile your code before use spark-submit!!!**
